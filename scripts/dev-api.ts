/**
 * Local dev API server — mirrors the Vercel function routes.
...
// Import business logic (uses process.env already loaded above)
import {
  ...
  sendResults,
} from '../api/_lib/competition-actions.js'
...
app.post('/api/auth/invite', async (req, res) => {
  ...
  try {
    const memberRes = await getPool().query(
      `SELECT id, first_name, last_name, email FROM members WHERE id = $1`, [member_id],
    )
    ...
    console.log(`\n[INVITE EMAIL] To: ${member.email}\nLink: ${appUrl}/set-password?token=${inviteToken}\n`)
    res.json({ ok: true })
  } catch (err) {
...
app.post('/api/auth/set-password', async (req, res) => {
  ...
  try {
    const result = await getPool().query(
      `SELECT id FROM admin_users WHERE invite_token = $1 AND invite_expires_at > NOW()`, [token],
    )
...
app.post('/api/competitions/:id/send-submission-reminders', async (req, res) => {
  if (!requireAuth(req, res)) return
  try {
    const result = await sendSubmissionReminders(req.params.id)
    res.json(result)
  } catch (err) {
    res.status(400).json({ error: err instanceof Error ? err.message : 'Error' })
  }
...
app.post('/api/competitions/:id/send-results', async (req, res) => {
  if (!requireAuth(req, res)) return
  try {
    const result = await sendResults(req.params.id)
    res.json(result)
  } catch (err) {
    res.status(400).json({ error: err instanceof Error ? err.message : 'Error' })
  }
...
app.post('/api/competitions/:id/send-judging-invite', async (req, res) => {
  if (!requireAuth(req, res)) return
  try {
    const result = await sendJudgingInvite(req.params.id)
    res.json(result)
  } catch (err) {
    res.status(400).json({ error: err instanceof Error ? err.message : 'Error' })
  }
...
// ── Submission portal routes (public) ─────────────────────────────────────────
...
app.post('/api/submit/:token/entries', async (req, res) => {
  ...
  if (!type || !['projim', 'printim'].includes(type)) {
    return void res.status(400).json({ error: 'type must be projim or printim' })
  }
...
app.post('/api/members', async (req, res) => {
  ...
  const result = await getPool().query(
    `INSERT INTO members (first_name,last_name,email,phone,membership_number,status,membership_type,experience_level,subs_paid,subs_due_date,annual_sub_amount,joined_date,privacy_act_ok,image_use_ok,club_rules_ok)
...
app.post('/api/members/:id/send-history-link', async (req, res) => {
  if (!requireAuth(req, res)) return
  try {
    await sendMemberHistoryLink(req.params.id)
    res.json({ ok: true })
  } catch (err) {
    res.status(400).json({ error: err instanceof Error ? err.message : 'Error' })
  }
...
app.post('/api/applicants', async (req, res) => {
  ...
  if (existing.rows.length > 0) {
    return void res.status(409).json({ error: 'An application with this email is already pending' })
  }
...
app.post('/api/applicants/:id/record-payment', async (req, res) => {
  ...
  const memberRes = await getPool().query(
    `INSERT INTO members (
     ...
     RETURNING id`,
    [ap.first_name, ap.last_name, ap.email, ap.phone, ap.annual_sub_amount, ap.privacy_act_ok, ap.image_use_ok, ap.club_rules_ok],
  )
  await getPool().query(
